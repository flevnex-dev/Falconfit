                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Payment Type ID Name</th>
                            <th class="text-center">Shipping Cost ID Name</th>
                    ...
                    <tbody>
                        @if(count($dataRow))
                            @foreach($dataRow as $row)  
                                <tr>
                                    <td class="text-center">{{$row->id}}</td><td class="text-center">{{$row->customer_id_name}}</td><td class="text-center">{{$row->total_amount}}</td><td class="text-center">{{$row->payable_amount}}</td><td class="text-center">{{$row->payment_type_id_name}}</td><td class="text-center">{{$row->shipping_cost_id_name}}</td>
                                    <td>{{formatDate($row->created_at)}}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{url('bookingorder/edit/'.$row->id)}}" type="button" class="btn btn-default">
                                                Edit 
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="{{url('bookingorder/delete/'.$row->id)}}" type="button" class="btn btn-default">
                                                Delete 
